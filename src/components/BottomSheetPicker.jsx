...
export default function BottomSheetPicker({ open, onClose, title, options, value, onChange }) {
  ...
  return (
    ...
    >
      <div className="bg-card rounded-t-3xl w-full max-w-md mx-auto pb-safe">
        <div className="flex justify-center pt-3 pb-1">
          <div className="w-10 h-1 bg-border rounded-full" />
        </div>
        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest px-5 py-3">{title}</p>
        ...
        <div className="px-5 py-4 border-t border-border" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
          <button onClick={onClose} className="w-full py-3.5 rounded-2xl border-2 border-border text-muted-foreground font-bold text-sm">
            Cancel
          </button>
        </div>
      </div>
